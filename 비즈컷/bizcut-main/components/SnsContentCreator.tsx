...
import {
    ...
    Loader2, Clock, Trash2, ChevronDown, ChevronUp, Instagram, Camera, Download, Upload, X, Check, Search
} from 'lucide-react';
...
const HISTORY_KEY = 'bizcut_content_history';
...
const SnsContentCreator: React.FC<SnsContentCreatorProps> = ({ initialTab }) => {
    const { apiKey, naverClientId, naverClientSecret } = useApiKey();
...
마크다운을 활용해 한국어로 출력해 주세요.
    ...
    const handleError = (err: any) => {
        console.error('BizCut Content Error Object:', err);
    ...
    const downloadCardNews = async () => {
        ...
        try {
            ...
            link.download = `bizcut-all-${Date.now()}.png`;
            link.href = dataUrl;
            link.click();
        } catch (err) {
    ...
    const downloadSingleSlide = async (index: number) => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.download = `bizcut-slide-${index + 1}-${Date.now()}.png`;
            link.href = dataUrl;
            link.click();
        } catch (err) {
    ...
    const renderCardNews = () => {
        ...
        let slides = [];
        try {
            // JSON 배열을 파싱 시도 (가끔 텍스트가 섞일 경우 방어 로직)
            const text = result.replace(/```json/g, '').replace(/```/g, '').trim();
        ...
        // 선택된 테마가 있으면 그것을 사용, 없으면 랜덤하게 하나를 배경으로 사용
        const cardBg = uploadedImages.length > 0 ? uploadedImages : (aiPhotoPrompts.length > 0 ? [aiPhotoPrompts[Math.floor(Math.random() * aiPhotoPrompts.length)].img] : []);

        return (
            <div className="mt-8 space-y-6 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex items-center justify-between">
                    <h3 className="font-black text-xl text-slate-800 flex items-center gap-2">
                        <Image className="text-orange-500" /> AI 디자인 완성본
                    </h3>
                    <div className="flex gap-2">
                        <button 
                            onClick={handleGenerateSlideImages} 
                            disabled={isGeneratingImages}
                            className={`flex items-center gap-2 px-4 py-2 ${isGeneratingImages ? 'bg-slate-200 text-slate-400' : 'bg-orange-500 text-white hover:bg-orange-600'} rounded-xl text-sm font-black shadow-lg shadow-orange-500/20 transition-all`}
                        >
                ...
                >
                    {slides.map((slide: any, idx: number) => {
                        ...
                        // 디자인 프리셋의 필터 및 오버레이
                        const filterStyle = themeInfo?.filter || 'none';
                        const overlayColor = themeInfo?.overlay || 'rgba(0,0,0,0.5)';
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* 레이어 2: 어두운 오버레이 (텍스트 가독성) */}
                                <div
                                    className="absolute inset-0 z-10 pointer-events-none"
                                    style={{ backgroundColor: backgroundImageUrl ? overlayColor : 'transparent' }}
                                />
                                ...
                                <div className="relative z-20 w-full h-full flex flex-col items-center justify-center p-10 text-center">
                                    ...
                                    <div className="space-y-6 w-full animate-in fade-in zoom-in-95 duration-500">
                                        <h2
                                            className={`tracking-tighter leading-[1.1] break-keep drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)] ${themeInfo?.font || 'font-black'} ${themeInfo?.color || 'text-white'} ${selectedTextTheme === 'big_sale' || selectedTextTheme === 'new_menu' ? 'text-4xl lg:text-5xl uppercase' : 'text-3xl lg:text-4xl'}`}
                                            style={(themeInfo as any)?.style}
                                        >
                                            {slide.title}
                                        </h2>

                                        <div className={`w-16 h-1.5 mx-auto rounded-full shadow-sm ${selectedTextTheme === 'strong' ? 'bg-orange-500' : 'bg-orange-500/80'}`} />

                                        <p
                                            className={`font-bold break-keep leading-relaxed text-lg drop-shadow-md px-2 ${selectedTextTheme === 'simple_beauty' ? 'text-emerald-950' : 'text-white/95'}`}
                                            style={selectedTextTheme === 'daily_life' ? { fontFamily: "'Nanum Myeongjo', serif" } : {}}
                                        >
                                            {slide.content}
                                        </p>
    ...
    const tabLabel = { caption: '캡션', cardnews: '카드뉴스', blog: '블로그' };
            ...
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
                ...
                <div className="text-center space-y-4">
                    ...
                    <h2 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">
                        상상하고 입력하면 <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-amber-500">완성</span>되는 콘텐츠
                    </h2>
                    {persona?.brandName && (
...
{/* Tab Switcher */}
                ...
                {tab === 'blog' && blogStep !== 'INIT' && (
                    <div className="flex items-center justify-between pb-4 border-b border-slate-200 px-4">
                        <div className={`flex flex-col items-center gap-1 ${['RESEARCHING', 'OUTLINING', 'OUTLINE_CONFIRM', 'DRAFTING', 'DONE'].includes(blogStep) ? 'text-indigo-600 font-bold' : 'text-slate-300'}`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded-full border-2 ${blogStep === 'RESEARCHING' ? 'border-indigo-600 animate-pulse' : 'border-indigo-600 bg-indigo-50'}`}>1</div>
                            <span className="text-xs">자료 조사</span>
                        </div>
                        <div className={`flex-1 h-0.5 mx-2 ${['OUTLINING', 'OUTLINE_CONFIRM', 'DRAFTING', 'DONE'].includes(blogStep) ? 'bg-indigo-600' : 'bg-slate-200'}`}></div>
                        <div className={`flex flex-col items-center gap-1 ${['OUTLINING', 'OUTLINE_CONFIRM', 'DRAFTING', 'DONE'].includes(blogStep) ? 'text-indigo-600 font-bold' : 'text-slate-300'}`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded-full border-2 ${blogStep === 'OUTLINING' ? 'border-indigo-600 animate-pulse' : ['OUTLINE_CONFIRM', 'DRAFTING', 'DONE'].includes(blogStep) ? 'border-indigo-600 bg-indigo-50' : 'border-slate-300'}`}>2</div>
                            <span className="text-xs">글 구성</span>
                        </div>
                        <div className={`flex-1 h-0.5 mx-2 ${['DRAFTING', 'DONE'].includes(blogStep) ? 'bg-indigo-600' : 'bg-slate-200'}`}></div>
                        <div className={`flex flex-col items-center gap-1 ${['DRAFTING', 'DONE'].includes(blogStep) ? 'text-indigo-600 font-bold' : 'text-slate-300'}`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded-full border-2 ${blogStep === 'DRAFTING' ? 'border-indigo-600 animate-pulse' : blogStep === 'DONE' ? 'border-indigo-600 bg-indigo-50' : 'border-slate-300'}`}>3</div>
                            <span className="text-xs">본문 작성</span>
                        </div>
                        <div className={`flex-1 h-0.5 mx-2 ${blogStep === 'DONE' ? 'bg-indigo-600' : 'bg-slate-200'}`}></div>
                        <div className={`flex flex-col items-center gap-1 ${blogStep === 'DONE' ? 'text-indigo-600 font-bold' : 'text-slate-300'}`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded-full border-2 ${blogStep === 'DONE' ? 'border-indigo-600 bg-indigo-50' : 'border-slate-300'}`}>4</div>
                                ...
                                {BLOG_THEMES.map(t => (
                                    <button
                                        ...
                                        className={`p-4 rounded-xl text-sm font-bold border-2 transition-all flex flex-col items-center gap-2 ${blogTheme === t.id
                                            ...
                                            : 'border-slate-100 bg-slate-50 text-slate-500 hover:border-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed'
                                            }`}
                                    >
